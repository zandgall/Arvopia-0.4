package pre.zandgall.tiles.utils;
...
public class Slider {
	...
	public Slider(Handler game, int min, int max, int val, boolean hv, String name) {
		this.game = game;
		
		
		this.min = min;
		ratio = Math.max(max / 10, 0.1);
		this.val = (int) ((val - min)/ratio*10);
		this.hv = hv;
		this.name = name;
...
//		mouseRight = game.getMouse().isRight();
	...
	public int getValue() {
		return (int) Math.round((val*ratio)/10+min);
	}
	
	public double getWholeValue() {
		return ((val*ratio)/10+min);
	}

	public void setValue(int val) {
		this.val = (int) ((val-min) / ((ratio*10)-min) * 100);
	}
