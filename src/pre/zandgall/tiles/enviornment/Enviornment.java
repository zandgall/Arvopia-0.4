package pre.zandgall.tiles.enviornment;
...
//	private double temp, tempChange, tempSwing;
	...
	public Enviornment(Handler handler, double wind, double windChange, double windSwing) {
		...
		sun = new Light(handler, 0,0,35, 1, Color.yellow);
	...
	public void render(Graphics g) {
		...
		for(int x = 0; x<game.getWidth(); x+=lightQuality) {
			for(int y = 0; y<game.getHeight(); y+=lightQuality) {
				...
				int alpha2 = (int) Public.range(lightManager.getClosest(x, y).getMax(), Public.range(0, 170, getTotalMinutes()-1100), (Public.dist(x,y,lightManager.getClosest(x, y).getX(), lightManager.getClosest(x, y).getY()) / (lightManager.getClosest(x, y).getStrength()/10)));
		...
		if(getMinutes() < 10) {
			g.drawString(":0"+getMinutes(), 30, game.getHeight()-20);
		} else {
			g.drawString(":"+getMinutes(), 30, game.getHeight()-20);
		}
